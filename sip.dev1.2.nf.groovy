...
def getCellIdentifier(path){
    path =  path.toString()
    if  (     path ==~ /^.*_1.*/       ){
        return new java.io.File(path.split('_1').join()).getName().split("\\.")[0]
    }
    if  (     path ==~ /^.*_2.*/       ){
        return new java.io.File(path.split('_2').join()).getName().split("\\.")[0]
    }
    if  (     path ==~ /^.*R1.*/       ){
        return new java.io.File(path.split('R1').join()).getName().split("\\.")[0]
    }
    if  (     path ==~ /^.*R2.*/       ){
        return new java.io.File(path.split('R2').join()).getName().split("\\.")[0]
    }
    return (new java.io.File(path).getName().split("\\.")[0])
}

//Channel
    //.fromFilePairs("${params.cellhome}/*.fastq", size: -1) 
    //.set{ReadChannel}


Channel
    .fromPath( ["${params.cellhome}/**/*.fastq", "${params.cellhome}/**/*.fq"] )
    .map{  it->[getCellIdentifier(it), it] }
